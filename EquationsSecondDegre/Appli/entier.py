...
def main_ent():
    ...
    if prgm.ValeurA.get()!=0:
        ...
        L1=Label(prgm.root,text='Erreur\u00A0:\u00A0a\u22600')
...
def sec_ent(a,b,c):
    ...
    beta_num=-delta
    beta_denom=4*a
    L1=Label(prgm.root,text='\u0394='+str(delta))
    L1.grid(column=6,row=0,sticky='w')
    ...
    if alpha_num%alpha_denom==0:
        ...
        if alpha_denom<0:
            L2=Label(prgm.root,text='\u03B1=('+str(int(-alpha_num))+')/('+str(int(-alpha_denom))+')='+str(alpha_num/alpha_denom).replace('.',','))
            L2.grid(column=6,row=1,sticky='w')
            prgm.rep[3]=L2.cget('text')
            prgm.rep[3]=L2.cget('text')
        else:
    ...
    if beta_num%beta_denom==0:
        ...
        while div_beta<=abs(beta_num) and div_beta<=abs(beta_denom):
            if beta_num%div_beta==0 and beta_denom%div_beta==0:
                ...
                div_beta=div_beta+1
        if beta_denom<0:
            L3=Label(prgm.root,text='\u03B2=('+str(int(-beta_num))+')/('+str(int(-beta_denom))+')='+str(beta_num/beta_denom).replace('.',','))
            L3.grid(column=6,row=2,sticky='w')
            prgm.rep[4]=L3.cget('text')
        else:
            L3=Label(prgm.root,text='\u03B2=('+str(int(beta_num))+')/('+str(int(beta_denom))+')='+str(beta_num/beta_denom).replace('.',','))
