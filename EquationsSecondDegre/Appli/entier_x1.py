...
def main(a,b,delta):
    if delta**0.5==int(delta**0.5):
        ...
        if x1_num%x1_denom==0:
            x1=x1_num//x1_denom
            L4=Label(prgm.root,text='x\u2081='+str(int(x1)))
            ...
            if x1_num<0 and x1_denom<0:
                L4=Label(prgm.root,text='x\u2081=('+str(int(-x1_num))+')/('+str(int(-x1_denom))+')='+str(x1_num/x1_denom).replace('.',','))
                L4.grid(column=6,row=3,sticky='w')
                prgm.rep[5]=L4.cget('text')
            else:
                L4=Label(prgm.root,text='x\u2081=('+str(int(x1_num))+')/('+str(int(x1_denom))+')='+str(x1_num/x1_denom).replace('.',','))
        ...
        if x1_num_int==0:
            if abs(x1_denom)==1:
                L4=Label(prgm.root,text='x\u2081=\u221A('+str(x1_num_rt)+')='+str(x1_num_rt**0.5).replace('.',','))
                L4.grid(column=6,row=3,sticky='w')
                prgm.rep[5]=L4.cget('text')
            else:
                if x1_denom<0:
                    L4=Label(prgm.root,text='x\u2081=\u221A('+str(x1_num_rt)+')/('+str(int(-x1_denom))+')='+str((x1_num_rt**0.5)/-x1_denom).replace('.',','))
                    L4.grid(column=6,row=3,sticky='w')
                    prgm.rep[5]=L4.cget('text')
                else:
                    L4=Label(prgm.root,text='x\u2081=-\u221A('+str(x1_num_rt)+')/('+str(int(x1_denom))+')='+str(-(x1_num_rt**0.5)/x1_denom).replace('.',','))
            ...
            if abs(x1_denom)==1:
                L4=Label(prgm.root,text='x\u2081='+str(int(-x1_num_int))+'+\u221A('+str(x1_num_rt)+')='+str(x1_num_int+x1_num_rt**0.5).replace('.',','))
                L4.grid(column=6,row=3,sticky='w')
                prgm.rep[5]=L4.cget('text')
            else:
                L4=Label(prgm.root,text='x\u2081=('+str(int(-x1_num_int))+'+\u221A('+str(x1_num_rt)+'))/('+str(int(-x1_denom))+')='+str((x1_num_int+x1_num_rt**0.5)/x1_denom).replace('.',','))
                L4.grid(column=6,row=3,sticky='w')
                prgm.rep[5]=L4.cget('text')
        else:
            if abs(x1_denom)==1:
